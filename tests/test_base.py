...
def test_versions_default_single_version():
    ...
    resource._register_version("get_foo", "1.0", path="/foo", model=dict)
...
def test_versions_multiple_with_deprecated():
    ...
    resource._register_version(
        "get_foo", "1.0", path="/v1/foo", model=dict, deprecated=True
    )
    resource._register_version("get_foo", "2.0", path="/v2/foo", model=dict)
...
def test_resolve_version_defaults_to_latest():
    resource = Resource(object())
    resource._register_version("get_foo", "1.0", path="/v1/foo", model=dict)
    resource._register_version("get_foo", "2.0", path="/v2/foo", model=dict)
...
def test_resolve_version_deprecated_warns():
    resource = Resource(object())
    resource._register_version(
        "get_foo", "1.0", path="/foo", model=dict, deprecated=True
    )
